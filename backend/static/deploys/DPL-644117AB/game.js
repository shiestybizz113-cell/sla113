// SLA113 Fish Shooter Engine v2 — Aztec Depths v4
// Production Visual Layer — Sprite-based + Canvas Fallback
const GAME_CONFIG = {
  "type": "fish_shooting",
  "name": "Aztec Depths v4",
  "version": "1.0.0",
  "built_by": "SLA113",
  "sprites": {
...
(async () => {
  ...
  // Background: use registered background image or gradient fallback
  if (GAME_CONFIG.background_url) {
    try {
      const bgTex = await PIXI.Texture.from(GAME_CONFIG.background_url, { crossOrigin: 'anonymous' });
      ...
      for (let y = 0; y < H(); y += 2) { const t = y / H(); oceanGfx.lineStyle(2, ((Math.floor(t * 12) << 16) | (Math.floor(16 + t * 16) << 8) | Math.floor(20 + t * 40))); oceanGfx.moveTo(0, y).lineTo(W(), y); }
