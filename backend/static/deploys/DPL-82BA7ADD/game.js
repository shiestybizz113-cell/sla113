...
(async () => {
  ...
  if(GAME_CONFIG.background_url){try{const img=new Image();img.crossOrigin='anonymous';img.src=GAME_CONFIG.background_url;await new Promise((r,e)=>{img.onload=r;img.onerror=e;setTimeout(e,8000);});
    // Crop bottom-right Gemini watermark (~5% corner) via canvas
    const cn=document.createElement('canvas');const cropPct=0.055;const cW=img.naturalWidth,cH=img.naturalHeight;cn.width=cW;cn.height=cH;const cx=cn.getContext('2d');cx.drawImage(img,0,0);
    // Mask bottom-right corner with adjacent pixels (cover watermark)
    const mW=Math.ceil(cW*cropPct),mH=Math.ceil(cH*cropPct);
    try{cx.drawImage(cn,cW-mW*2,cH-mH*2,mW,mH,cW-mW,cH-mH,mW,mH);}catch(_){}
