// SLA113 Fish Shooter v6 — Shadow Pack
...
const GAME_CONFIG = {
  ...
  "built_by": "SLA113",
  "sprites": {
    "southern_lifestyle_loader": {
      "sprite_url": "/api/sla113/sprites/proxy?url=https://customer-assets.emergentagent.com/job_3653cf8a-8710-488d-846f-2f0428b714dd/artifacts/pi354pey_image.jpeg",
      "frame_width": 1080,
      "frame_height": 1080,
      "columns": 1,
      "rows": 1,
    ...
    "slam_members_only": {
      "sprite_url": "/api/sla113/sprites/proxy?url=https://customer-assets.emergentagent.com/job_3653cf8a-8710-488d-846f-2f0428b714dd/artifacts/r7z583sm_image.jpeg",
      "frame_width": 1920,
      "frame_height": 1920,
      "columns": 1,
      "rows": 1,
    ...
    "southernlyfestyle_arcade_lobby": {
      "sprite_url": "/api/sla113/sprites/proxy?url=https://customer-assets.emergentagent.com/job_3653cf8a-8710-488d-846f-2f0428b714dd/artifacts/d6oehyc3_image.png",
      "frame_width": 2048,
      "frame_height": 2048,
      "columns": 1,
      "rows": 1,
  ...
  "loader_url": "/api/sla113/sprites/proxy?url=https://customer-assets.emergentagent.com/job_3653cf8a-8710-488d-846f-2f0428b714dd/artifacts/pi354pey_image.jpeg",
...
(async () => {
  ...
  const BOSSES_ALL=[
    ...
    {name:'G-WOLF',color:0xffd700,hp:350,val:2500,sz:88,sprite:'g_wolf'},
    {name:'XOLOTL PACK',color:0xffaa00,hp:600,val:4000,sz:100,sprite:'wolf_xolotl_pack'},
    {name:'JAGUAR ELITE',color:0xff6600,hp:700,val:6000,sz:110,sprite:'jaguar_warrior_elite'},
    {name:'JAGUAR CHAMPION',color:0xff2244,hp:900,val:10000,sz:120,sprite:'jaguar_warrior_champion'}
  ];
  // Filter by lobby config if present: show only main + partner + extras
  const LOB=GAME_CONFIG.lobby||{};
  const wanted=new Set([LOB.main_boss,LOB.partner_boss,...(LOB.extra_bosses||[])].filter(Boolean));
  const BOSSES=wanted.size?BOSSES_ALL.filter(b=>wanted.has(b.sprite)):BOSSES_ALL;
  ...
  function shake(intensity,duration){shakeAmt=Math.max(shakeAmt,intensity);shakeDecay=intensity/duration;}
  app.ticker.add(()=>{
    if(shakeAmt>0){
      const ox=(Math.random()-0.5)*shakeAmt,oy=(Math.random()-0.5)*shakeAmt;
      ['bg','mid','fish','nets','bullets','fx'].forEach(n=>{L[n].x=ox;L[n].y=oy;});
      shakeAmt-=shakeDecay*(app.ticker.deltaMS||16);if(shakeAmt<0){shakeAmt=0;['bg','mid','fish','nets','bullets','fx'].forEach(n=>{L[n].x=0;L[n].y=0;});}
    }
  });

  function playBossIntro(type,sn,onDone){
    playRoar(type.color);shake(22,1400);
    const ov=new PIXI.Container();ov.zIndex=900;L.ui.addChild(ov);
    const hex='#'+type.color.toString(16).padStart(6,'0');
    // Dim overlay
    const dim=new PIXI.Graphics();dim.beginFill(0x000000,0.78).drawRect(0,0,W(),H()).endFill();dim.alpha=0;ov.addChild(dim);
    // Radial color flash (canvas texture)
    const fc=document.createElement('canvas');fc.width=Math.max(W(),512);fc.height=Math.max(H(),512);
    const fx=fc.getContext('2d');const gx=fx.createRadialGradient(fc.width/2,fc.height/2,0,fc.width/2,fc.height/2,Math.max(fc.width,fc.height)*0.65);
    gx.addColorStop(0,hex);gx.addColorStop(0.35,hex+'66');gx.addColorStop(1,'rgba(0,0,0,0)');
    fx.fillStyle=gx;fx.fillRect(0,0,fc.width,fc.height);
    const flash=new PIXI.Sprite(PIXI.Texture.from(fc));flash.width=W();flash.height=H();flash.alpha=0;flash.blendMode=PIXI.BLEND_MODES.ADD;ov.addChild(flash);
    // Diagonal Aztec banner bars
    const b1=new PIXI.Graphics();b1.beginFill(type.color,0.22).drawPolygon([-W(),H()*0.4,W()*2,H()*0.3,W()*2,H()*0.55,-W(),H()*0.65]).endFill();b1.alpha=0;ov.addChild(b1);
    const b2=new PIXI.Graphics();b2.beginFill(0x000000,0.55).drawPolygon([-W(),H()*0.42,W()*2,H()*0.35,W()*2,H()*0.5,-W(),H()*0.58]).endFill();b2.alpha=0;ov.addChild(b2);
    // Hero sprite (large, centered)
    const hero=new PIXI.Container();hero.x=W()/2;hero.y=H()*0.46;
    if(loadedTex[sn]){
      const td=loadedTex[sn];
      const fr=td.anims.ultra||td.anims.attack||td.anims.run||td.anims.walk||td.anims.idle||td.frames.slice(0,4);
      const a=new PIXI.AnimatedSprite(fr);a.anchor.set(0.5);a.animationSpeed=0.15;a.play();
      const heroScale=(H()*0.55)/td.cfg.frame_height;a.scale.set(heroScale);hero.addChild(a);
    }else{
      const s=new PIXI.Sprite(bossTex[BOSSES.indexOf(type)]);s.anchor.set(0.5);s.scale.set(3.2);hero.addChild(s);
    }
    hero.scale.set(0.01);hero.alpha=0;ov.addChild(hero);
    // Expanding ring shockwave
    const ring=new PIXI.Graphics();ring.lineStyle(6,type.color,1).drawCircle(0,0,40);ring.x=W()/2;ring.y=H()*0.46;ring.alpha=0;ring.scale.set(0.1);ov.addChild(ring);
    const ring2=new PIXI.Graphics();ring2.lineStyle(3,0xffffff,1).drawCircle(0,0,40);ring2.x=W()/2;ring2.y=H()*0.46;ring2.alpha=0;ring2.scale.set(0.1);ov.addChild(ring2);
    // "BOSS INBOUND" subtitle
    const sub=new PIXI.Text('BOSS INBOUND',{fontSize:22,fill:type.color,fontFamily:'monospace',fontWeight:'bold',letterSpacing:14,stroke:0x000000,strokeThickness:4});sub.anchor.set(0.5);sub.x=W()/2;sub.y=H()*0.78;sub.alpha=0;ov.addChild(sub);
    // Main title
    const title=new PIXI.Text(type.name,{fontSize:72,fill:0xffffff,fontFamily:'monospace',fontWeight:'bold',letterSpacing:8,stroke:hex,strokeThickness:8,dropShadow:true,dropShadowColor:type.color,dropShadowBlur:24,dropShadowAlpha:0.9,dropShadowDistance:0});title.anchor.set(0.5);title.x=W()/2;title.y=H()*0.86;title.alpha=0;ov.addChild(title);
    // Reward badge
    const rwd=new PIXI.Text('x'+type.val+' JACKPOT',{fontSize:16,fill:0xd4af37,fontFamily:'monospace',fontWeight:'bold',letterSpacing:6,stroke:0x000000,strokeThickness:3});rwd.anchor.set(0.5);rwd.x=W()/2;rwd.y=H()*0.92;rwd.alpha=0;ov.addChild(rwd);

    let t=0;const D=1800;
    const tick=()=>{
      t+=app.ticker.deltaMS||16;const p=Math.min(t/D,1);
      // Dim fade
      dim.alpha=p<0.12?(p/0.12)*0.78:p>0.82?((1-p)/0.18)*0.78:0.78;
      // Flash peak early
      flash.alpha=p<0.25?Math.sin(p/0.25*Math.PI)*0.9:0;
      // Banners slide in
      if(p<0.3){b1.alpha=p/0.3*0.9;b2.alpha=p/0.3;b1.x=-W()+p/0.3*W();b2.x=W()-p/0.3*W();}
      else if(p>0.82){b1.alpha=b2.alpha=(1-p)/0.18;}
      else{b1.x=0;b2.x=0;b1.alpha=0.9;b2.alpha=1;}
      // Hero punch-in + hover + fade-out
      if(p<0.22){const e=p/0.22;hero.scale.set(0.01+e*1.18);hero.alpha=e;}
      else if(p<0.32){hero.scale.set(1.19-((p-0.22)/0.1)*0.19);hero.alpha=1;}
      else if(p<0.82){hero.scale.set(1+Math.sin(p*25)*0.02);hero.alpha=1;}
      else{const f=(1-p)/0.18;hero.scale.set(f*1);hero.alpha=f;}
      // Rings expand outward then fade
      const rp=Math.min(p/0.55,1);
      ring.scale.set(0.1+rp*16);ring.alpha=(1-rp)*0.9;
      ring2.scale.set(0.1+rp*20);ring2.alpha=(1-rp);
      // Sub + Title + Reward stagger
      if(p>=0.28){const tp=Math.min((p-0.28)/0.18,1);sub.alpha=tp;}
      if(p>=0.32){const tp=Math.min((p-0.32)/0.2,1);title.alpha=tp;title.scale.set(1+(1-tp)*0.4);}
      if(p>=0.48){const tp=Math.min((p-0.48)/0.18,1);rwd.alpha=tp;}
      if(p>0.82){const fo=(1-p)/0.18;sub.alpha=title.alpha=rwd.alpha=fo;}
      if(p>=1){app.ticker.remove(tick);L.ui.removeChild(ov);if(onDone)onDone();}
    };
    app.ticker.add(tick);
  }
  ...
  function spawnBoss(){
    ...
    f.visible=false;
    L.fish.addChild(f);fishes.push(f);
    playBossIntro(type,sn,()=>{if(f.alive){f.visible=true;announce(`BOSS: ${type.name}`,type.color);}});
  }
